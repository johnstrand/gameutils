...
public class ShuffleBagTests
{
    ...
    public void Constructor_EmptyItems_ThrowsArgumentException()
    {
        ...
        try
        {
            new ShuffleBag<string>(Array.Empty<(string, int)>());
    ...
    [TestMethod]
    public void Constructor_InvalidWeight_ThrowsArgumentOutOfRangeException()
    {
        // Arrange
        var invalidItems = new[] { ("A", 0) };

        // Act & Assert
        try
        {
            new ShuffleBag<string>(invalidItems);
